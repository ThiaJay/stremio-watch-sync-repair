...
export class Runtime{
 ...
 summary(plan){return {id:plan.id,kind:plan.kind,digest:plan.digest,expires:plan.expires,checked:plan.checked,operations:plan.operations.map(o=>({id:o.id??o.key,target:o.target??'stremio',desired:o.desired,fields:o.allowed,dateEstimated:o.dateEstimated})),skipped:plan.skipped??[],conflicts:plan.conflicts??[],remaining:plan.remaining??0,status:plan.status};}
 ...
 async scheduledPass(profile,due=new Map(),generation=this.generation){
  ...
  if(profile.modules.traktBridge){
   tasks.push({id:'traktBridge',minutes:profile.trakt.intervalMinutes,run:async()=>{
    const plan=await this.plan(profile.id,'sync');
    if(profile.trakt.stremioWriteEnabled&&plan.operations.length&&plan.remaining===0&&!plan.conflicts.length&&plan.operations.every(op=>op.target==='stremio'&&op.desired===true))await this.apply(profile.id,plan.id,plan.digest);
   }});
  }
  for(const task of tasks){
   if(generation!==this.generation||!this.config.server.scheduleEnabled)break;
   if((due.get(task.id)??0)>Date.now())continue;
   try{await task.run();due.set(task.id,Date.now()+task.minutes*60000);}
